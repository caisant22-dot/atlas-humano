...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError('Este navegador não conseguiu iniciar o visualizador 3D. Tente um navegador com WebGL habilitado.');return;}
  ...
  renderer.domElement.setAttribute('aria-label','Anatomia humana interativa. Arraste para orbitar, pinça ou rolagem para zoom, e toque em uma estrutura para inspecioná-la.');
  ...
  const labelLayer=document.createElement('div');labelLayer.className='labels-layer';el.appendChild(labelLayer);let labelEls:HTMLElement[]=[];
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('Não foi possível montar a geometria anatômica.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:'Não foi possível carregar a anatomia.');}})();
  ...
  const animate=()=>{
   ...
   if(changed||moving||lastExtent<0){
    ...
    if(s.quizHighlight)for(const id of s.quizHighlight)selection.add(id);
   ...
   if(dirty){renderer.render(scene,camera);targets=[];if(amount>.45){const hasSolid=atlas.parts.some((p,i)=>p.system!=='integumentary'&&data[i*4+3]>.5);atlas.parts.forEach((p,i)=>{if(data[i*4+3]<.5||(hasSolid&&p.system==='integumentary'))return;let left=Infinity,right=-Infinity,top=Infinity,bottom=-Infinity;for(let corner=0;corner<8;corner++){projected.set(p.bounds[(corner&1)?1:0][0]+data[i*4],p.bounds[(corner&2)?1:0][1]+data[i*4+1],p.bounds[(corner&4)?1:0][2]+data[i*4+2]).project(camera);const x=(projected.x+1)*el.clientWidth/2,y=(1-projected.y)*el.clientHeight/2;left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y);}projected.copy(centers[i]).add(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])).project(camera);if(projected.z< -1||projected.z>1)return;targets.push({index:i,x:(projected.x+1)*el.clientWidth/2,y:(1-projected.y)*el.clientHeight/2,left,right,top,bottom});});}
    // Labels overlay
    if(s.labelsVisible&&ready){const maxLabels=30,placed:{x:number;y:number;w:number;h:number}[]=[];const visibleIndices:number[]=[];atlas.parts.forEach((p,i)=>{if(data[i*4+3]>.5&&p.system!=='integumentary')visibleIndices.push(i);});const sorted=visibleIndices.map(i=>{projected.copy(centers[i]).add(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])).project(camera);return{i,z:projected.z,x:(projected.x+1)*el.clientWidth/2,y:(1-projected.y)*el.clientHeight/2};}).filter(t=>t.z>-1&&t.z<1&&t.x>20&&t.x<el.clientWidth-20&&t.y>40&&t.y<el.clientHeight-60).sort((a,b)=>a.z-b.z);let labelIdx=0;for(const t of sorted){if(labelIdx>=maxLabels)break;const w=80,h=24;const overlaps=placed.some(p=>Math.abs(p.x-t.x)<(p.w+w)/2&&Math.abs(p.y-t.y)<(p.h+h)/2);if(overlaps)continue;let lbl=labelEls[labelIdx];if(!lbl){lbl=document.createElement('div');lbl.className='anatomy-label';lbl.innerHTML='<span class="label-line"></span><span class="label-text"></span>';labelLayer.appendChild(lbl);labelEls.push(lbl);}lbl.style.left=`${t.x}px`;lbl.style.top=`${t.y-8}px`;lbl.style.display='';(lbl.lastElementChild as HTMLElement).textContent=atlas.parts[t.i].name;placed.push({x:t.x,y:t.y,w,h});labelIdx++;}for(let j=labelIdx;j<labelEls.length;j++)labelEls[j].style.display='none';}else{for(const l of labelEls)l.style.display='none';}
    dirty=false;}

  };animate();
  const contextLost=(e:Event)=>{e.preventDefault();onError('A sessão 3D foi pausada pelo seu dispositivo. Recarregue para continuar.');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
